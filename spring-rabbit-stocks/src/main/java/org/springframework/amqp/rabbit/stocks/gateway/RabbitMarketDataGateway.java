 ...
 * @author Mark Fisher
 */
public class RabbitMarketDataGateway extends RabbitGatewaySupport implements
		MarketDataGateway {

	private static Log logger = LogFactory
			.getLog(RabbitMarketDataGateway.class);
	...
	public void sendMarketData() {
		Quote quote = generateFakeQuote();// 将股票按照原来的basePrice进行包装后得到一个新的报价信息
		Stock stock = quote.getStock();
		logger.info("Sending Market Data for " + stock.getTicker());
		String routingKey = "app.stock.quotes." + stock.getStockExchange()
				+ "." + stock.getTicker();
	...
	private static class MockStock extends Stock {
		...
		private MockStock(String ticker, StockExchange stockExchange,
				int basePrice) {
		...
		private String randomPrice() {
			return this.twoPlacesFormat.format(this.basePrice
